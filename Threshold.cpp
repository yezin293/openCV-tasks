...
// 일반 Threshold 이진화 함수 (원본 영상도 함께 출력)
void runThresholding() {
    ...
    // threshold() 함수를 사용하여 이진화 수행
    // threshold(입력 영상, 출력 영상, 임계값, 최대값, 임계값 적용 방법)
    // → 임계값 100을 기준으로:
    //    - 100 이상이면 255(흰색)
    //    - 100 이하면 0(검은색)으로 변환 (THRESH_BINARY 적용)
    ...
    //창 위치 조정 (원본 영상 왼쪽, 이진화된 영상 오른쪽 배치)
    moveWindow("Original Image", 100, 100);  // 원본 영상 창을 (100,100)에 배치
    moveWindow("Binary Image", 750, 100);   // 이진화된 영상을 오른쪽(750,100)에 배치
